...
Each local app should be capable of building its own small team. That team can include local virtual agents, proxy agents for real people, and temporary helpers for scoped tasks. The local team remains simple and shallow. Larger organizational complexity comes from connecting many local teams together.
...
- each local app is a whole team desk
- each local app is also a node in a larger organization
...
A node is one local Holon app or hosted Holon desk owned by a real person or team.
...
The owner should not have to manage a deep internal org chart. The local app should make it easy to create, configure, pause, archive, and replace staff.
