...
Without handoff, proxy work becomes just "send a task to a URL." That is too weak for a workplace product.
...
- target_type: owner | local_ai_staff | proxy_staff | remote_node
...
- receiver can delegate to local AI staff
- receiver cannot forward to another proxy unless explicitly allowed
...
MVP should implement handoff enough to support proxy staff reliably:

- handoff table or equivalent persisted model
- handoff id on outbound proxy assignments
